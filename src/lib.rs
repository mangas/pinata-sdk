...
impl PinataApi {
    ...
    pub async fn pin_by_hash(&self, hash: PinByHash) -> Result<PinByHashResult, ApiError> {
        let response = self
            .client
            .post(&api_url("/pinning/pinByHash"))
            .json(&hash)
            .send()
            .await?;
    ...
    /// Pin any content as a file to Pinata's IPFS nodes.
    ///
    ///
    pub async fn pin_file_content(&self, pin_data: PinContent) -> Result<PinnedObject, ApiError> {
        let mut form = pin_data.files.into_iter().fold(Form::new(), |form, file| {
            form.part("file", Part::bytes(file.content).file_name(file.name))
        });
    ...
    pub async fn pin_file(&self, pin_data: PinByFile) -> Result<PinnedObject, ApiError> {
        let x: PinContent = pin_data.try_into()?;
        self.pin_file_content(x).await
    }

    async fn pin_content(&self, form: Form) -> Result<PinnedObject, ApiError> {
        let response = self
            .client
