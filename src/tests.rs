...
async fn test_set_hash_pin_policy_works() {
    ...
    let result = get_api()
        .set_hash_pin_policy(HashPinPolicy::new(
    ...
    match result {
        Ok(data) => {
            ...
            assert_eq!(
                data.rows.len(),
                0,
                "Zero count should also return zero rows"
            );
        }
        Err(e) => assert!(false, "{}", e),
    }
...
async fn test_pin_json_to_ipfs() {
    ...
    let result = get_api()
        .pin_json(PinByJson::new(TestData {
    ...
    match result {
        Ok(data) => {
            ...
            assert_eq!(
                data.ipfs_hash,
                "QmcDRRZ8Sy2QrpN8VySimHH5SToSPScW8yP8VmkZ2gDEJv"
            );
...
async fn test_pin_json_to_ipfs_with_metadata() {
    ...
    match result {
        Ok(data) => {
            ...
            assert_eq!(
                data.ipfs_hash,
                "QmScaKE4777guCGczz6giVSMX2QJhxAdBeXCqPnsivQE8f"
            );
...
async fn test_pin_file_to_ipfs() {
    let result = get_api().pin_file(PinByFile::new("./test-file.txt")).await;
    ...
    match result {
        Ok(data) => {
            ...
            assert_eq!(
                data.ipfs_hash,
                "QmYW6YYCco35LGEmpm6oyJVijjTR5fPxvKxRULEauefXNH"
            );
...
async fn test_pin_directory_to_ipfs() {
    let result = get_api().pin_file(PinByFile::new("./test-dir")).await;
    ...
    match result {
        Ok(data) => {
            ...
            assert_eq!(
                data.ipfs_hash,
                "QmYTyd2A15snZbRbWi2cbZkis45DzDdPSdzdF3wXdMEWVk"
            );
...
async fn test_unpin() {
    ...
    let pin_result = api
        .pin_json(PinByJson::new(PinData { random: "Tell me" }))
        .await
        .unwrap();
...
async fn test_change_hash_metadata_pin_querying_works() {
    ...
    let pin_result = api
        .pin_json(
            PinByJson::new(PinData {
                random: "Custom metadata",
            })
    ...
    new_metadata.insert(
        "new_value".to_string(),
        MetadataValue::String("awesome".into()),
    );
...
async fn test_pin_content() {
    ...
    let res = api
        .pin_file_content(PinContent {
            files: vec![file],
            pinata_metadata: None,
            pinata_option: None,
        })
        .await
        .expect("failed to pin file content");

    assert_eq!(
        res.ipfs_hash,
        String::from("Qmf412jQZiuVUtdgnB36FXFX7xg5V6KEbSJ4dpQuhkLyfD")
    );

    api.unpin(&res.ipfs_hash)
        .await
        .expect("failed to unpin hash")
}
