use crate::{
    api::metadata::{MetadataKeyValues, MetadataValue, PinListMetadata, PinMetadata},
    ApiError,
};
...
/// let api = PinataApi::new("api_key", "secret_api_key").unwrap();
///
/// let result = api.pin_file_content(PinByFile::new("file_or_dir_path").try_into()?).await;
///
/// if let Ok(pinned_object) = result {
///   let hash = pinned_object.ipfs_hash;
/// }
...
impl PinContent {
    ...
    /// let api = PinataApi::new("api_key", "secret_api_key").unwrap();
    ///
    /// let req = PinContent::new(vec![FileContent {
    ///     name: "some_file".into(),
    ///     content: "hello world".as_bytes().to_vec(),
    /// }])
    ///
    /// let result = api.pin_file_content(req).await;
    ///
    /// if let Ok(pinned_object) = result {
    ///   let hash = pinned_object.ipfs_hash;
    /// }
...
impl TryInto<PinContent> for PinByFile {
    type Error = ApiError;

    fn try_into(self) -> Result<PinContent, Self::Error> {
        let PinByFile {
            files,
            pinata_metadata,
            pinata_option,
